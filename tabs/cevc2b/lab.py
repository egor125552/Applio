...
def cevc2b_lab_tab():
    ...
    gr.Markdown(
        ...
        "This stage creates deterministic same-phrase pseudo-pairs from existing "
        "clean slices. It does not ask for or record new audio. Real rough and "
        "mixed slices remain natural references for the critic."
    )
    with gr.Row():
    ...
    with gr.Accordion("Dataset preparation settings", open=False):
        ...
        gr.Markdown(
            ...
            "All variants preserve the source sample count and approximately "
            "preserve RMS. Existing output under `cevc2b/pseudo_pairs` is replaced."
        )

    run = gr.Button("Prepare Experiment 2B pseudo-pairs")
    status = gr.Textbox(label="Experiment 2B status")
    ...
    run.click(
        ...
        outputs=[status, manifest, source, weak, medium, strong],
    )
